    ...
    <div style={{minHeight:'100vh',background:'#F4F6FB',fontFamily:"'Plus Jakarta Sans',sans-serif"}}>
      ...
      <style>{`
        ...
        @media (max-width: 640px){
          ...
          .report-title{font-size:24px !important; line-height:1.2 !important}
          .report-subtitle{font-size:12px !important}
          .print-btn{width:100% !important; justify-content:center !important}
          .summary-grid,.user-breakdown-grid{grid-template-columns:1fr !important; gap:12px !important}
          .success-panel{padding:18px !important}
          .success-row{flex-direction:column !important; align-items:flex-start !important; gap:10px !important}
          .success-rate{font-size:36px !important}
          ...
          .table-shell tr{padding:10px 0 !important; border-bottom:1px solid #F4F6FB !important}
          ...
          .user-name-cell{flex-direction:column !important; align-items:flex-start !important}
        }
        @media print{nav,button{display:none!important}}
      `}</style>
      <Navbar/>
      <div className="page-shell" style={{maxWidth:1100,margin:'0 auto',padding:'36px 28px'}} id="report-area">
        {/* Header */}
        <div className="au d1 report-header" style={{display:'flex',alignItems:'flex-start',justifyContent:'space-between',marginBottom:28,flexWrap:'wrap',gap:12}}>
          <div>
            <h1 className="report-title" style={{fontFamily:'Fraunces,serif',fontSize:30,fontWeight:800,color:'#0F1C35',marginBottom:4,display:'flex',alignItems:'center',gap:10}}>
              <FileText size={26} style={{color:'#1B3A6B'}}/> System Reports
            </h1>
            <p className="report-subtitle" style={{fontSize:14,color:'#8A96A8'}}>Platform activity overview — generated {new Date().toLocaleDateString()}</p>
          </div>
        ...
        {loading ? <div style={{textAlign:'center',padding:60,color:'#8A96A8'}}>Loading report data...</div> : <>
          ...
          {/* Summary Cards */}
          <div className="summary-grid" style={{display:'grid',gridTemplateColumns:'repeat(4,1fr)',gap:16,marginBottom:24}}>
            {[
          ...
          <div className="au d2" style={{background:'white',borderRadius:18,border:'1px solid #D8E2F0',padding:24,marginBottom:24,overflow:'hidden',position:'relative'}}>
            ...
            <div className="user-breakdown-grid" style={{display:'grid',gridTemplateColumns:'repeat(3,1fr)',gap:14,textAlign:'center'}}>
          ...
          {/* Success Rate */}
          <div className="au d3 success-panel" style={{background:'white',borderRadius:18,border:'1px solid #D8E2F0',padding:24,marginBottom:24}}>
            <div className="success-row" style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:14,gap:12}}>
              <div>
              ...
              <span className="success-rate" style={{fontFamily:'Fraunces,serif',fontSize:42,fontWeight:800,background:'linear-gradient(135deg,#1B3A6B,#D4A017)',WebkitBackgroundClip:'text',WebkitTextFillColor:'transparent'}}>{rate}%</span>
            </div>
                ...
                <tbody>
                  {users.map(u=>(
                    <tr key={u._id} className="trow" style={{borderBottom:'1px solid #F4F6FB'}}>
                      <td data-label="Name" style={{padding:'12px 18px'}}>
                        <div className="user-name-cell" style={{display:'flex',alignItems:'center',gap:9}}>
                      ...
                      <td data-label="Email" style={{padding:'12px 18px',fontSize:13,color:'#4A5568'}}>{u.email}</td>
                      <td data-label="Role" style={{padding:'12px 18px'}}>{roleBadge(u.role)}</td>
                      <td data-label="Joined" style={{padding:'12px 18px',fontSize:13,color:'#8A96A8'}}>{new Date(u.createdAt).toLocaleDateString()}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
